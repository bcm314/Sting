...
public:
  ...
  bool in_check() const;
  bool in_threat(int ply) const; 
  Square long_king() const;
  ...
  bool move_is_pseudo_legal(const Move m) const;
  ...
  bool possible_fortress(Color c) const;

  // Doing and undoing moves
  void do_setup_move(Move m, StateInfo& newSt);
  ...
  bool move_is_reversed(Move m) const;
  bool original_king_square(Color c, Square s, bool previous) const;
...
inline bool Position::in_threat(int ply) const {
	return ply-1 > st->pliesFromNull;
}

inline Square Position::long_king() const {
	return st->previous->previous->previous->previous->ksq;
}

inline bool Position::original_king_square(Color c, Square s, bool previous) const {
	return !bit_is_set(previous ? st->previous->king[c] : st->king[c], s);
}
...
inline bool Position::move_is_passed_pawn_push(Move m) const {
   ...
   return   type_of_piece_on(move_from(m)) == PAWN
...
inline bool Position::move_is_capture_or_promotion(Move m) const {
  ...
  // Move must not be MOVE_NONE !
  return (m & (3 << 14)) ? !move_is_castle(m) : !square_is_empty(move_to(m));
}
