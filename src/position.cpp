...
const Value Position::seeValues[] = {
    VALUE_ZERO,
    PawnValueMidgame, BishopValueMidgame, BishopValueMidgame,
    RookValueMidgame, QueenValueMidgame, VALUE_ZERO
};
...
*/
  ...
  st->king[WHITE] = 1ULL << king_square(WHITE);
  st->king[BLACK] = 1ULL << king_square(BLACK);
  st->ksq = king_square(opposite_color(sideToMove));
  st->pinned = hidden_checkers<true>(sideToMove);
  return;
...
Bitboard Position::pinned_pieces(Color c) const {
	...
	return (c == sideToMove ? st->pinned : hidden_checkers<true>(c));
}
...
bool Position::possible_fortress(Color c) const {	 
	
	if (   non_pawn_material(WHITE) == BishopValueMidgame 
		&& non_pawn_material(BLACK) == BishopValueMidgame
		&& opposite_colored_bishops())
		return true;
	
	if (non_pawn_material(c) + 2 * PawnValueMidgame > non_pawn_material(opposite_color(c)))
		return false;	

	Bitboard b =((pieces(PAWN, WHITE) << 9) & ~FileABB) | ((pieces(PAWN, WHITE) << 7) & ~FileHBB);
	if (b & pieces(PAWN, BLACK))
		return false;

	if (   piece_count(WHITE, PAWN) + piece_count(BLACK, PAWN) == 16
		&& (pieces(PAWN, WHITE) << 8) == pieces(PAWN, BLACK))
		return true;

	if (    piece_count(opposite_color(c), PAWN) <= 1
		|| (bit_is_set(Border, king_square(opposite_color(c))) && square_distance(king_square(c), king_square(opposite_color(c))) == 2))
		return true;	

	if (non_pawn_material(opposite_color(c)) > QueenValueMidgame)
		return false;

	b = pieces(PAWN, opposite_color(c));
	if (((b << 1) & ~FileABB) & pieces(PAWN, opposite_color(c)))
		return false;

	b &= ~((c == WHITE ? occupied_squares() << 8 : occupied_squares() >> 8) & b);
	if (b)
	{
		Bitboard bo = c == WHITE ? pieces(PAWN, c) << 8 : pieces(PAWN, c) >> 8;		
		bo |= (((bo << 1) & ~FileABB) | ((bo >> 1) & ~FileHBB));
		bo |= (c == WHITE ? pieces(PAWN, c) << 16 : pieces(PAWN, c) >> 16);		
		b &= ~bo;		
		while(b)
		{				
			if (((b << 1) & ~FileABB) & pieces(PAWN, opposite_color(c)))
			...
			b = c == WHITE ? b >> 8 : b << 8;
			b &= ~(bo | pieces(PAWN, opposite_color(c)));	

			if (b & (Rank1BB | Rank8BB))
				return false;
		}		
	}
...
bool Position::pl_move_is_legal(Move m) const {
  ...
  return   !bit_is_set(st->pinned, from)
...
bool Position::pl_move_is_evasion(Move m) const
{
  ...
  // En-passant captures are verified in pl_move_is_legal()
  if (move_is_ep(m))
  ...
  if (type_of_piece_on(from) == KING)
  {
	  Bitboard b = occupied_squares();
	  clear_bit(&b, from);
	  return !(attackers_to(to, b) & pieces_of_color(opposite_color(us)));		 
  } 	 
  ...
  return bit_is_set(target, to) && pl_move_is_legal(m);
}
...
bool Position::move_is_pseudo_legal(const Move m) const {
  ...
  if (piece_on(to) != PIECE_NONE && color_of_piece_on(to) == us)
      return false;
  ...
  return in_check() ? pl_move_is_evasion(m) : pl_move_is_legal(m);
}
...
void Position::do_move(Move m, StateInfo& newSt, const CheckInfo& ci, bool moveIsCheck) {
  ...
  assert(move_is_ok(m));
  assert(&newSt != st);
  assert(st->npMaterial[WHITE] == compute_non_pawn_material(WHITE));
  assert(st->npMaterial[BLACK] == compute_non_pawn_material(BLACK));
  ...
  st->ksq = king_square(sideToMove);  
  if (pt == KING)
	  st->king[sideToMove] |= attacks_from<KING>(from);  // Finish
...
bool Position::move_is_reversed(Move m) const {
	
	Square from = move_from(m);
	if (type_of_piece_on(from) == PAWN || move_is_capture(m))
		return false;
	Square to = move_to(m);
	//PieceType pt = type_of_piece_on(from);
	StateInfo* stp = st->previous;

	while (stp != NULL && stp->capturedType <= PAWN)
	{
		if (move_is_ok(stp->move) && move_to(stp->move) == from)
		{
			if (stp->capturedType != PIECE_TYPE_NONE)
			return false;
		
			if (move_from(stp->move) == to)
				return true;
			else
			{
				//if (pt == KING)
				//	return false;
				from = move_from(stp->move);
			}
		}
		stp = stp->previous;
		if (stp != NULL)
...
int Position::see_sign(Move m) const {
  ...
  if (seeValues[type_of_piece_on(to)] >= seeValues[type_of_piece_on(from)])
      return 1;
...
void Position::init_zobrist() {
  ...
  zobSideToMove = 1ULL << 63;

  for (i = 0; i < 2; i++) for (j = 0; j < 8; j++) for (k = 0; k < 64; k++)
      zobrist[i][j][k] = rk.rand<Key>() & ~zobSideToMove;

  for (i = 0; i < 64; i++)
      zobEp[i] = rk.rand<Key>() & ~zobSideToMove;

  for (i = 0; i < 16; i++)
      zobCastle[i] = rk.rand<Key>() & ~zobSideToMove;
  
  zobExclusion  = rk.rand<Key>() & ~zobSideToMove;
}
