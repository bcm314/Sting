...
namespace {
  ...
  struct EvalInfo {
	...
	PieceAttack attackedByPiece[16];
	PieceAttack* pieceAttack;	
  ...
  const Score WeightsInternal[] = {
         S(248, 271), S(252, 259), S(46, 0), S(247, 247/6), S(259, 259/6)
  };  
  ...
  const int TrappedPieceMobility[] = { 0, 0, 0, 0, 1, 6 }; 
  ...
  void trapped_pieces(const Position& pos, EvalInfo& ei, Score& mobilityWhite, Score& mobilityBlack);
  bool stalemate(const Position& pos, EvalInfo& ei);
  bool possible_stalemate(const Position& pos, EvalInfo& ei, Color c);
...
Value do_evaluate(const Position& pos, Value& margin) {
  ...
  EvalInfo ei;
  ei.pieceAttack = ei.attackedByPiece;
  ei.pieceAttack[0].attackedBy = 0;
  Value margins[2];
  ...
  score = pos.value() + (pos.side_to_move() == WHITE ? Tempo : -Tempo);
  ...
  {
	  Value result = ei.mi->evaluate(pos);
	  if (result != VALUE_NONE)
	  {
		  margin = VALUE_ZERO;
		  return Max(VALUE_MATED_IN_PLY_MAX + 2, Min(VALUE_MATE_IN_PLY_MAX - 2, result));
	  }
  }
  ...
  if (ei.mi->game_phase() <= PHASE_TRAPPED || ei.attackedByPiece[0].attackedBy)
	  trapped_pieces(pos, ei, mobilityWhite, mobilityBlack);  
  ...
  if (ei.mi->game_phase() <= PHASE_HI_TRAPPED)
  {
	  if (stalemate(pos, ei))
		  return VALUE_ZERO;

	  if (   sf == SCALE_FACTOR_NORMAL &&
		  (  (eg_value(score) <= -PawnValueMidgame && possible_stalemate(pos, ei, WHITE)) 
		  || (eg_value(score) >=  PawnValueMidgame && possible_stalemate(pos, ei, BLACK))) )
	  {
		  Value v = eg_value(score) > 0 ?  PawnValueMidgame + (eg_value(score) - PawnValueMidgame)/16 
			                            : -PawnValueMidgame + (eg_value(score) + PawnValueMidgame)/16; 
		  return (pos.side_to_move() == WHITE ? 1 : -1) * v;
	  }
		    
	  if (pos.piece_count(WHITE, PAWN) >= 6 && pos.piece_count(BLACK, PAWN) >= 6)
	  {
		 int c = count_1s<Max15>((pos.pieces(PAWN, WHITE) << 8) & pos.pieces(PAWN, BLACK));
		 if (c >= 6)
			 return (pos.side_to_move() == WHITE ? 1 : -1) * eg_value(score)/(c == 8 ? 32 : 8);
	  }	 
  }    
  ...
  if (   ei.mi->game_phase() < PHASE_MIDGAME
	  ...
	  && abs(pos.non_pawn_material(WHITE) - pos.non_pawn_material(BLACK)) < (ei.mi->game_phase() < Phase(32) ? RookValueMidgame : PawnValueMidgame))	 
  {
  ...
  v = Max(VALUE_MATED_IN_PLY_MAX + 2, Min(VALUE_MATE_IN_PLY_MAX - 2, v));

  // In case of tracing add all single evaluation contributions for both white and black
  if (Trace)
  {
      trace_add(PST, pos.value() + (pos.side_to_move() == WHITE ? Tempo : -Tempo));
...
namespace {
  ...
  Score evaluate_pieces(const Position& pos, EvalInfo& ei, Score& mobility, Bitboard mobilityArea) {
    ...
    while ((s = *ptr++) != SQ_NONE)
    {
		...
		if (   (ei.mi->game_phase() <= PHASE_HI_TRAPPED && !mob)
			|| (ei.mi->game_phase() <= PHASE_TRAPPED && (mob <= TrappedPieceMobility[Piece] || bit_is_set(Border, s))))
		{
			ei.pieceAttack->attackedBy = b;
			ei.pieceAttack->pieceType = Piece;
			ei.pieceAttack->color = Us;
			ei.pieceAttack->mobility = mob;
			ei.pieceAttack->square = s;
			(++ei.pieceAttack)->attackedBy = 0;							
		}
    ...
    do {
        ...
        if (rr)
        {
            ...
            if (pos.square_is_empty(blockSq))
            {
                ...
                if (   (squares_in_front_of(Them, s) & pos.pieces(ROOK, QUEEN, Them))
                    ...
                    unsafeSquares = squaresToQueen & (ei.attackedBy[Them][0] | pos.pieces_of_color(Them)) & ~ei.attackedBy[Us][PAWN];
        ...
        if (square_file(s) == FILE_A || square_file(s) == FILE_H)
        {
            if (pos.non_pawn_material(Them) <= KnightValueMidgame)
                ...
                ebonus -= ebonus / 4;
			if (s + pawn_push(Us) == pos.king_square(Us))
				ebonus = ebonus / 2;
        }

		if (   !pos.non_pawn_material(Us)
			&& !ei.pi->passed_pawns(Them) 
			&&  pos.non_pawn_material(Them) <= RookValueMidgame
			&& (ei.pi->passed_pawns(Us) & (ei.pi->passed_pawns(Us) - 1)))
			ebonus *= 2;
  ...
  Score evaluate_unstoppable_pawns(const Position& pos, EvalInfo& ei) {
    ...
    for (c = WHITE; c <= BLACK; c++)
    {
        ...
        while (b)
        {
			...
			if (queeningPath & pos.occupied_squares())
				continue;
  ...
  void trapped_pieces(const Position& pos, EvalInfo& ei, Score& mobilityWhite, Score& mobilityBlack) {
	  ...
	  bool trappedKing[2] = {false, false};
	  Value penalty;
	  Bitboard b;
	  
	  if (pos.non_pawn_material(BLACK))
	  {
		  b = ei.attackedBy[WHITE][KING] & ~(pos.pieces_of_color(WHITE) | ei.attackedBy[BLACK][0]);
		  if (!b)
		  {
			  trappedKing[WHITE] = true;
			  mobilityWhite += make_score(0, -400);
		  }
	  }
	  if (pos.non_pawn_material(WHITE))
	  {
		  b = ei.attackedBy[BLACK][KING] & ~(pos.pieces_of_color(BLACK) | ei.attackedBy[WHITE][0]);
		  if (!b)
		  {
			  trappedKing[BLACK] = true;
			  mobilityBlack += make_score(0, -400);
		  }
	  }		
	  
	  PieceAttack* pieceAttack = ei.attackedByPiece;
	  while ((*pieceAttack).attackedBy)
	  {
		  Color c = (*pieceAttack).color;
		  Color oc = opposite_color(c);	
		  if (pos.non_pawn_material(c) + BishopValueMidgame < pos.non_pawn_material(oc))
		  {
			  pieceAttack++;
			  continue;
		  }
		  PieceType pt = (*pieceAttack).pieceType;		  

		  if (    pt == QUEEN 
			  &&  ei.mi->game_phase() <= PHASE_TRAPPED
			  && (*pieceAttack).mobility <= TrappedPieceMobility[pt] 
		                                 - (trappedKing[c] ? 0 : ei.mi->game_phase() <= PHASE_LO_TRAPPED ? 1 : 3)
										 - (pos.non_pawn_material(c) - KnightValueMidgame >= pos.non_pawn_material(opposite_color(c)) ? 0 : 4))			 			 
		  {
			  penalty = -PieceValueEndgame[pt]/(trappedKing[c] ? 1 : 2);
			  c == WHITE ? mobilityWhite += make_score(0, penalty) : mobilityBlack += make_score(0, penalty);
			 
			  pieceAttack++;
			  continue;
		  }

		  b = (*pieceAttack).attackedBy;		  	  			  
		  if (trappedKing[c])
			  b &= ~pos.pieces_of_color(c);
		  else b &= ~pos.pieces(PAWN, c);

		  if (!(b & ~ei.attackedBy[oc][0]))				 
		  {	
			  if (trappedKing[oc] && (ei.attackedBy[oc][KING] & (*pieceAttack).attackedBy))
			  {
				  pieceAttack++;
				  continue;
			  }

			  b &= ~(ei.attackedBy[oc][PAWN]);
			  if (pt == QUEEN)
				  b &= ~(ei.attackedBy[oc][KNIGHT] | ei.attackedBy[oc][BISHOP] | ei.attackedBy[oc][ROOK]);
			  else if (pt == ROOK)
				  b &= ~(ei.attackedBy[oc][KNIGHT] | ei.attackedBy[oc][BISHOP]);
			  else if ((*pieceAttack).attackedBy & pos.pieces_of_color(oc) & ~pos.pieces(PAWN, oc))
			  {
				  pieceAttack++;
				  continue;
			  }

			  for (PieceType pt2 = KING; pt2 >= PAWN; pt2--)
			  {						  
				  if (pt2 == pt)
					  continue;
				  else if (b & ei.attackedBy[c][pt2]) 
					  break;
				  else if (pt2 > PAWN) continue;
				 
				  penalty = -PieceValueEndgame[pt]/(trappedKing[c] ? 1 : 2);
				  if (relative_rank(c, (*pieceAttack).square) > RANK_2 && bit_is_set(ei.pi->passed_pawns(c), (*pieceAttack).square) - pawn_push(c))
					  penalty += -PawnValueEndgame/(trappedKing[c] ? 1 : 2);				  
				  c == WHITE ? mobilityWhite += make_score(0, penalty) : mobilityBlack += make_score(0, penalty);			 
			  }			 
		  }
		  pieceAttack++;
  ...
  bool stalemate(const Position& pos, EvalInfo& ei) {
	...
	if (pos.piece_count(c, QUEEN) >= 1)			
		return false;	

	if ((ei.attackedBy[c][PAWN] & pos.pieces_of_color(opposite_color(c))) || ((c == WHITE ? pos.pieces(PAWN, c) << 8 : pos.pieces(PAWN, c) >> 8) & ~pos.occupied_squares()))
		return false;

	if (ei.attackedBy[c][KING] & ~(pos.pieces_of_color(c) | ei.attackedBy[opposite_color(c)][0]))
		return false;	
	
	Bitboard pinned = pos.pinned_pieces(c);

	for (PieceType pt = ROOK; pt > PAWN; pt--)
	{
		if ((ei.attackedBy[c][pt] & pos.pieces_of_color(c)) == ei.attackedBy[c][pt])
			continue;
		
		if (pos.piece_count(c, pt) > 1)
			return false;

		Bitboard p = pos.pieces(pt, c);		

		if (!pinned || !(pinned & p))
			return false;

		Square s = pop_1st_bit(&p);

		if (!bit_is_set(pinned, s) || (squares_between(s, pos.king_square(c)) & ei.attackedBy[c][pt]))
			return false;		
	}
  ...
  bool possible_stalemate(const Position& pos, EvalInfo& ei, Color c) {
	...
	if (!pos.non_pawn_material(c) || pos.non_pawn_material(c) > RookValueMidgame + BishopValueMidgame)			
		return false;

	if (pos.non_pawn_material(c) > RookValueMidgame && !pos.pieces(ROOK, c))
		return false;

	if (ei.attackedBy[c][KING] & ~(pos.pieces(PAWN, c) | ei.attackedBy[opposite_color(c)][0]))
		return false;

	if (   (ei.attackedBy[c][PAWN] & pos.pieces_of_color(opposite_color(c))) 
		|| (pos.pieces(PAWN, c) && (c == WHITE ? pos.pieces(PAWN, c) << 8 : pos.pieces(PAWN, c) >> 8) & ~(pos.pieces(PAWN, KING) | pos.pieces_of_color(opposite_color(c)))) )
		return false;		
