...
MovePicker::MovePicker(const Position& p, Move ttm, const History& h,
                       SearchStack* ss) : pos(p), H(h) {  

  ttMoves[0].move = ttm ? ttm : (ss->mateKiller && !p.in_check() && p.move_is_pseudo_legal(ss->mateKiller)) ? ss->mateKiller : MOVE_NONE; 
  ttMoves[1].move = (!ss->mateKiller || p.in_check() || !ttm || ss->mateKiller == ttm || !p.move_is_pseudo_legal(ss->mateKiller)) ? MOVE_NONE : ss->mateKiller;
  int searchTT = ttMoves[0].move;
...
MovePicker::MovePicker(const Position& p, Move ttm, Depth d, const History& h)
                      : pos(p), H(h) {  
  ttMoves[0].move = ttm ? ttm : MOVE_NONE;
  int searchTT = ttMoves[0].move;
  ...
  else
  {
      ...
      if (ttm != MOVE_NONE && !p.move_is_capture_or_promotion(ttm))
...
void MovePicker::score_captures() {
  ...
  for (MoveStack* cur = moves; cur != lastMove; cur++)
  {
	  ...
	  if (move_is_promotion(m))
          cur->score += PieceValueMidgame[move_promotion_piece(m)] - PawnValueMidgame;
	  if (move_is_ep(m))
		  cur->score += PawnValueMidgame;
  }
}
...
Move MovePicker::get_next_move() {
  ...
  while (true)
  {
      ...
      case PH_KILLERS:
          move = (curMove++)->move;
          if (move != MOVE_NONE)
			  if (   pos.move_is_pseudo_legal(move)
				  && move != ttMoves[0].move
				  && move != ttMoves[1].move
				  && !pos.move_is_capture_or_promotion(move))
				  return move;
			  else if (move == killers[0].move)
				  killers[0].move = MOVE_NONE;
			  else if (move == killers[1].move)
				  killers[1].move = MOVE_NONE;
          break;
