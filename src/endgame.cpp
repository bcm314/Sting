...
Value Endgame<Value, KXK>::apply(const Position& pos) const {
  ...
  if ((loserKSq == SQ_A1 || loserKSq == SQ_H1 || loserKSq == SQ_A8 || loserKSq == SQ_H8)
	  && pos.piece_count(strongerSide, QUEEN) 
	  && square_distance(loserKSq, pos.piece_list(strongerSide, QUEEN, 0)) == 2
	  && weakerSide == pos.side_to_move())
	  return VALUE_ZERO;
...
template<>
Value Endgame<Value, KQKP>::apply(const Position& pos) const {
  ...
  Square queenSq = pos.piece_list(strongerSide, QUEEN, 0);  

  if (   relative_rank(weakerSide, pawnSq) == RANK_7
	  && bit_is_set(mask7pawn, pawnSq)
	  && queenSq != pawnSq + pawn_push(weakerSide))
  {
	  Square winnerKSq = pos.king_square(strongerSide);
	  Value result = 2 * PawnValueMidgame + 56 - square_distance(winnerKSq, pawnSq) * 8;
...
ScaleFactor Endgame<ScaleFactor, KBPsK>::apply(const Position& pos) const {
      ...
      && (pawns & ~file_bb(pawnFile)) == EmptyBoardBB)
  {
      ...
      if (   opposite_color_squares(queeningSq, bishopSq)
		  && square_distance(queeningSq, kingSq) <= relative_rank(weakerSide, pos.piece_list(strongerSide, PAWN, 0)) )         
		  return square_distance(kingSq, queeningSq) <= 1 ? SCALE_FACTOR_ZERO : SCALE_FACTOR_QUART;      
  }
